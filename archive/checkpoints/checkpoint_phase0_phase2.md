# Pre-Dispatch Return / RTO Risk Engine — Checkpoint: Phases 0–2

**Status:** Phases 0, 1, and 2 complete. Phase 3 (full Returnformer: Graph Transformer + Node2Vec
+ attention fusion) not yet started.
...
reconstructed from memory once Phase 3 is underway. Intended to be extended as later phases
...
## 5. Engineering Artifacts Produced
...
## 6. Documented Deviations from the Paper (for transparency)
...
## 7. Running Scoreboard

| Model | Phase | Overall AUC | Cold-customer AUC |
|---|---|---|---|
| CatBoost / tuned trees | 1 | 0.833 | ~0.65 |
| GraphSAGE (Option 1, warm) | 2 | 0.855 | 0.798 |
| GraphSAGE (Option 2, real cold-start) | 2 | 0.876 | 0.767 |
| Returnformer (Graph Transformer core) | 3 | *TBD* | *TBD — the key number* |

The number to watch in Phase 3 is whether the richer architecture widens the cold-customer lift
beyond GraphSAGE's 0.767.

## 8. Next: Phase 3 Preview

Phase 3 builds the core Returnformer on top of the now-validated Phase 2 pipeline, in three
milestones, each a complete stopping point:

1. **Graph Transformer** — swap the GraphSAGE layers for PyG's `TransformerConv` (2 layers,
   4 heads, per the paper), keeping the same graph object, harness, and both eval modes.
2. **Node2Vec structural embeddings** — computed on the (small) graph, capturing each node's
   topological position independent of its own features.
3. **Attention fusion** — the ~30-line module from the paper's Equations 1–4 that adaptively
   combines original features with the Node2Vec structural embeddings before they enter the encoder.

Stopping after milestone 3 yields a legitimate reproduction of the paper's core method — graph-based
return prediction with topological embeddings — benchmarked against both the 0.833 tabular baseline
and the 0.876 / 0.767 GraphSAGE results above. GEA and KAN remain an optional Phase 4 stretch tier
(see Section 4.1 for why GEA is unnecessary at single-subgraph scale). India adaptation and the
synthetic-data generator remain a later, separate phase.
